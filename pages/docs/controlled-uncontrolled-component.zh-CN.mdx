...
不过，需要注意的是这里的 `div` 可不是原生 `DOM`。

在 `JSX` 语法中，`HTML` 标签都是 `React` 元素，也就是根据真实 `DOM` 结构抽象出来的对象，即我们常说的虚拟 `DOM`。
...
`babel` 将 `JSX` 元素转换成了一个虚拟 `DOM` 的创建操作，函数 `_jsx` 创建的虚拟 `DOM` 结构是这样的：
...
注意这些内置浏览器组件并不是原生 `DOM`，它们是 `React` 元素。在 `JSX` 部分我们已经讲过了。

下面我们看一个受控浏览器组件的例子：

浏览器组件 `<input>` 的状态 `value` 完全由 `onChange` 事件控制。
...
浏览器组件 `<input>` 的状态完全由组件自身管理，我们仅通过 `ref` 就能获取它的值，组件的数据更新不需要外部控制。
...
function UncontrolledInput() {
  ...
  return (
    <div>
      ...
      <button onClick={handleButtonClick}>取值</button>
    </div>
  )
}
```
