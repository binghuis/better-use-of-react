...
全部类型查看 [@types/react 声明文件](https://github.com/DefinitelyTyped/DefinitelyTyped/blob/master/types/react/index.d.ts)。

**描述组件类型**

- `React.ReactElement` - 表示一个 `JSX` 元素。
- `React.ReactNode` - 表示一个 `JSX` 元素或部分基本类型。
...
**描述事件类型**
[TS everyday type](https://www.typescriptlang.org/docs/handbook/2/everyday-types.html)
[React TypeScript 备忘录](https://github.com/typescript-cheatsheets/react#useful-react-prop-type-examples)
