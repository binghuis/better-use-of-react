...
将一个 `ref` 对象绑定到一个固定 `DOM` 上很简单，但当 `DOM` 是动态生成的列表呢，如何将这些 `DOM` 绑定到 `ref` 上？

使用 `ref` 回调可以解决这个问题，`ref` 回调是一个参数为 `DOM` 节点的函数，当组件挂载或卸载时被调用。

下面是一个示例，我实现了一个 `useMultiRef` 自定义 hook，用于绑定多个 `ref`。
